...
alt.on('keyup', async (key) => {
  ...
  if (key === 65) { // A
    const debugCamState = alt.getMeta('debugCamState');
    if (debugCamState) return alt.emitRaw('request:debug_setDebugCamX', -0.5);
  }
  ...
  if (key === 87) { // W
    const debugCamState = alt.getMeta('debugCamState');
    if (debugCamState) return alt.emitRaw('request:debug_setDebugCamY', 0.5);
  }
  ...
  if (key === 32) { // SPACE
    const debugCamState = alt.getMeta('debugCamState');
    if (debugCamState) return alt.emitRaw('request:debug_setDebugCamZ', 0.5);
  }
